...
class Media:
    ...
    def get_newest_post_metadata(self, user_id: int) -> dict:
        """
        Retrieves the metadata of the newest post from the specified user.
        """
        newest_post_metadata = self.instagram_client.user_medias(user_id, amount=1)
        newest_post_metadata = newest_post_metadata[0].model_dump()
        logging.info("Retrieved newest post metadata")
        return newest_post_metadata

    def get_post_date(self, post_metadata: dict) -> str:
        """
        Retrieves the date from the given post metadata and returns it in the format YYYY-MM-DD.
        """
        post_date = post_metadata["taken_at"]
        formatted_date = post_date.strftime("%Y-%m-%d")
        logging.info(f"Newest post date: {formatted_date}")
        return formatted_date

    def get_rating(self, post_metadata: dict) -> str:
        """
        Retrieves the rating from the given post metadata.
        """
        rating_pattern = r"(\d|\d(\.|,)\d+)\/10"
        try:
            rating = re.search(rating_pattern, post_metadata["caption_text"]).group()
    ...
    def get_photo(self, post_metadata: dict) -> Path:
        """
        Downloads the photo from the given post metadata and returns the path.
        """
        media_pk = post_metadata["pk"]
